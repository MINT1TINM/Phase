      ...
      <v-sheet height="750">
        ...
        >
          <v-card elevation="10" min-width="100px">
            ...
            <v-card-text>
              <p>{{ `所属项目: ${selectedEvent.project_name}` }}</p>
              <p>{{ `所属过程: ${selectedEvent.process_name}` }}</p>
              <p>
            ...
            <v-card-actions>
              ...
              <!-- <user-chip class="my-2" :userID="selectedEvent.username"></user-chip> -->
              <v-spacer></v-spacer>
              <v-btn
                rounded
                text
...
export default class ProjectDashboardCalendar extends Vue {
  ...
  @Prop({ default: () => [] }) taskList!: any[];
  @Prop({ default: () => [] }) goToProjectTask!: Function;

  calendarTaskList: any[] = [];
  focus = '';
  type = 'month';
  typeToLabel = {
    month: 'Month',
  ...
  async mounted() {
    ...
    let filters: any = this.$root.$options.filters;
    // console.log(this.taskList);
    this.taskList.forEach(task => {
      // task.name = task.task_name;
      task.startDate = filters.format(task.startDate, 'yyyy-MM-dd');
      task.endDate = filters.format(task.endDate, 'yyyy-MM-dd');
      this.calendarTaskList.push(task);
    });
  }
}
</script>
