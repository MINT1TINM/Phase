        ...
        <template v-slot:default>
          ...
          <tbody>
            <tr v-for="(item,i) in billList" :key="`user-${i}`">
              ...
              <td class="caption text-left">¥ {{ item.price }}</td>
              <td>
                <v-btn outlined small rounded color="success darken-1">
                  <v-icon size="15">mdi-check</v-icon>&nbsp;通过
                </v-btn>
                <v-btn class="ml-3" outlined small rounded color="error">
                  <v-icon size="15">mdi-close</v-icon>&nbsp;退回
                </v-btn>
              </td>
            </tr>
          </tbody>
...
@Component
export default class AuditQuery extends Vue {
  private status = '未完成';
